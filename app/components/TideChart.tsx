...
import {
	getAdmiraltyTideData,
	getSunriseSunsetData,
} from "../utils/admiraltyApi";
...
export default function TideChart({
	...
	const loadTideData = async () => {
		try {
			...
			console.log(
				`🌊 Loading tide data for ${spotName} (${latitude}, ${longitude}) - ${showDays} days from ${startDate.toISOString()}`
			);
			...
			console.log(
				`🌊 Loaded ${tideResponse.tideEvents.length} tide events from ${tideResponse.source}`
			);
			...
			for (let i = 0; i < showDays; i++) {
				...
				try {
					const sunTimes = await getSunriseSunsetData(
						latitude,
						longitude,
						date
					);
			...
			console.log(
				`🌅 Loaded sun data for ${Object.keys(sunDataMap).length} days`
			);
		} catch (err) {
	...
	const chartData = useMemo(() => {
		console.log(
			`📊 Generating chart data: ${tideData.length} tide events, ${Object.keys(sunData).length} sun days, target date: ${startDate.toISOString()}`
		);
		...
		for (let minute = 0; minute < totalMinutes; minute += minuteInterval) {
			...
			if (beforeEvent && afterEvent) {
				...
				for (const event of tideData) {
					...
					if (
						Math.abs(eventTime.getTime() - currentTimeMs) <=
						15 * 60 * 1000
					) {
				...
				const nearestAfterEvent = tideData.find(
					(event) =>
						new Date(event.time).getTime() >
						new Date(beforeEvent.time).getTime()
				);
				...
				if (nearestAfterEvent) {
					...
					if (
						...
						nearestAfterEvent.type === "low"
					) {
						// Falling tide: continue smooth fall
						smoothExtension =
							(1 - Math.cos(timeExtension * Math.PI)) / 2;
					} else if (
						beforeEvent.type === "low" &&
						nearestAfterEvent.type === "high"
					) {
						// Rising tide: continue smooth rise
						smoothExtension =
							(1 + Math.cos((1 - timeExtension) * Math.PI)) / 2;
					} else {
					...
					const heightDifference =
						nearestAfterEvent.height - beforeEvent.height;
					const extrapolatedHeight =
						beforeEvent.height + heightDifference * smoothExtension;

					tideLevel =
						heightRange > 0
							? (extrapolatedHeight - minHeight) / heightRange
							: 0.5;
				...
				const nearestBeforeEvent = [...tideData]
					.reverse()
					.find(
						(event) =>
							new Date(event.time).getTime() <
							new Date(afterEvent.time).getTime()
					);

				if (nearestBeforeEvent) {
					// Calculate the trend direction and continue the curve backwards
					const beforeTime = new Date(
						nearestBeforeEvent.time
					).getTime();
					const afterTime = new Date(afterEvent.time).getTime();
					const timeExtension = Math.min(
						(afterTime - currentTimeMs) / (afterTime - beforeTime),
						1.0
					);
					...
					let smoothProgress;
					if (
						nearestBeforeEvent.type === "high" &&
						afterEvent.type === "low"
					) {
						// Falling tide: continue smooth fall backwards
						smoothProgress = (1 - Math.cos(progress * Math.PI)) / 2;
					} else if (
						nearestBeforeEvent.type === "low" &&
						afterEvent.type === "high"
					) {
					...
					const heightDifference =
						afterEvent.height - nearestBeforeEvent.height;
					const extrapolatedHeight =
						nearestBeforeEvent.height +
						heightDifference * smoothProgress;
			...
			const isNow =
				isToday &&
				Math.abs(currentTimeMs - actualNow.getTime()) <
			...
			if (sunTimes) {
				...
				isDaylight =
					currentTimeMs >= sunriseTime && currentTimeMs <= sunsetTime;
				isSunrise =
					Math.abs(currentTimeMs - sunriseTime) < 30 * 60 * 1000; // Within 30 minutes
				isSunset =
					Math.abs(currentTimeMs - sunsetTime) < 30 * 60 * 1000; // Within 30 minutes
			}
	...
	const generateMarkers = () => {
		const markers = [];
		console.log(
			`🔍 Generating markers with chartData length: ${chartData.length}`
		);
		...
		if (isToday) {
			...
			if (currentPoints.length > 0) {
				...
				console.log(
					`🕐 Adding current time marker at: ${currentPoint.time}`
				);
				markers.push(
					<ReferenceLine
						key="current-time"
		...
		console.log(
			`🗓️ Looking for events between ${targetDateStart.toISOString()} and ${targetDateEnd.toISOString()}`
		);
		...
		const targetDateEvents = tideData.filter((event) => {
			...
			const isOnTargetDate =
				eventDate.getTime() === targetDateStart.getTime();

			console.log(
				`🔍 Checking event at ${event.time} (${eventTime.toISOString()}) - event date: ${eventDate.toISOString()}, target date: ${targetDateStart.toISOString()}, isOnTargetDate: ${isOnTargetDate}`
			);
		...
		console.log(
			`📅 Found ${targetDateEvents.length} events for target date:`,
			targetDateEvents
		);
		...
		targetDateEvents.forEach((event, index) => {
			if (event.type === "high" || event.type === "low") {
				uniqueTideEvents.set(
					`${event.type}-${event.time}-${index}`,
					event
				);
			}
		});
		...
		console.log(
			`🏔️ All tide events for target date:`,
			Array.from(uniqueTideEvents.values())
		);

		console.log(
			`🌊 Found ${uniqueTideEvents.size} unique tide events:`,
			Array.from(uniqueTideEvents.keys())
		);
		...
		uniqueTideEvents.forEach((event, key) => {
			...
			console.log(
				`🌊 Adding tide marker for ${event.type} at ${timeString}`
			);
			...
			chartData.forEach((point) => {
				const [pointHours, pointMinutes] = point.time
					.split(":")
					.map(Number);
			...
			console.log(
				`🎯 Found closest point for ${timeString}: ${closestPoint.time} (diff: ${smallestDiff} minutes)`
			);
		...
		console.log(
			`🌅 Looking for sun data for ${targetDateStr}, available dates:`,
			Object.keys(sunData)
		);
		...
		console.log(
			`🌊 Generated ${uniqueTideEvents.size} tide markers and ${uniqueSunEvents.size} sun markers`
		);
		...
		uniqueSunEvents.forEach((sunEvent, key) => {
			...
			console.log(
				`🌅 Found closest point for ${sunEvent.type} at ${sunEvent.time}: ${closestPoint.time} (diff: ${smallestDiff} minutes)`
			);
		...
		if (sunTimes) {
			...
			if (sunriseTime.toDateString() === startDate.toDateString()) {
				...
				chartData.forEach((point) => {
					...
					const pointTotalMinutes = pointHours * 60 + pointMinutes;
					const diff = Math.abs(
						pointTotalMinutes - sunriseTotalMinutes
					);
				...
				console.log(
					`🌙 Added morning night area: ${startTime} to ${closestSunrisePoint.time} (sunrise closest match)`
				);
			}
			...
			if (sunsetTime.toDateString() === startDate.toDateString()) {
				...
				chartData.forEach((point) => {
					...
					const pointTotalMinutes = pointHours * 60 + pointMinutes;
					const diff = Math.abs(
						pointTotalMinutes - sunsetTotalMinutes
					);
				...
				console.log(
					`🌙 Added evening night area: ${closestSunsetPoint.time} (sunset closest match) to ${endTime} (last data point)`
				);
			}
		} else {
			console.log(
				`❌ No sun times found for night areas on ${targetDateStr}`
			);
		}
		...
		return (
			<div className={`bg-white rounded-lg p-4 ${className}`}>
				<div className="text-center text-gray-600">
					...
					<p className="text-sm">
						Tide events: {tideData?.length || 0}
					</p>
					<p className="text-sm">
						Sun data days: {Object.keys(sunData).length}
					</p>
				</div>
			</div>
		);
	...
	console.log(
		`📊 Rendering TideChart for ${spotName}: ${tideData.length} tide events, ${Object.keys(sunData).length} sun days`
	);

	// Determine if this is live data - admiralty_uk is fully live, enhanced_calculation is acceptable fallback
	const isLiveData =
		dataSource === "admiralty_uk" || dataSource === "enhanced_calculation";
		...
		>
			<div className="flex justify-between items-center mb-4">
				...
				<div className="text-sm text-gray-600">
					{showDays === 1 ? "Today" : `${showDays} Days`} •{" "}
					{isLiveData ? "UK Admiralty" : "Demo Data"}
				</div>
			</div>
