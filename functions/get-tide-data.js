export async function handler(event, context) {
  ...
  try {
    const { lat, lng, startDate: startDateParam, endDate: endDateParam } = event.queryStringParameters || {};
    ...
    console.log(`🌊 Tide data request: ${latitude}, ${longitude} from ${startDate.toISOString()} to ${endDate.toISOString()}`);
    ...
    if (admiraltyApiKey) {
      try {
        ...
        const admiraltyUrl = new URL('https://admiraltyapi.azure-api.net/uktidalapi/api/V1/Stations/NearestStation');
        admiraltyUrl.searchParams.set('longitude', longitude.toString());
        ...
        // Get tide events
        const eventsUrl = new URL('https://admiraltyapi.azure-api.net/uktidalapi/api/V1/Stations/TidalEvents');
        ...
        if (eventsData && Array.isArray(eventsData)) {
          for (const event of eventsData) {
            ...
            if (eventDate >= startDate && eventDate <= endDate) {
              tideEvents.push({
                time: event.DateTime,
                type: event.EventType?.toLowerCase() === 'highwater' ? 'high' : 'low',
        ...
        console.log(`✅ Processed ${tideEvents.length} tide events for date range`);
    ...
    while (currentTime < endTime) {
      ...
      const times = [
        ...
        { type: 'high', minutes: baseHighTime1 + variation, height: 4.8 },
        { type: 'low', minutes: baseLowTime2 + variation, height: 1.5 },
        { type: 'high', minutes: baseHighTime2 + variation, height: 4.5 }
      ];
    ...
    console.log(`📊 Generated ${tideEvents.length} enhanced calculation events`);
