			...
			res.on("end", () => {
				try {
					...
					if (result.tideData) {
						console.log("\n🌊 TIDE DATA:");
						console.log(
							`📈 Direction: ${result.tideData.isRising ? "🔼 RISING ❌" : "🔽 FALLING ✅"}`
						);
						console.log(
							`📊 Level: ${(result.tideData.currentLevel * 100).toFixed(1)}%`
						);
						console.log(
							`📡 Source: ${result.tideData.source || "unknown"}`
						);

						if (result.tideData.nextHigh) {
							console.log(
								`⏰ Next High: ${new Date(result.tideData.nextHigh).toLocaleString("en-GB", { timeZone: "Europe/London" })} BST`
							);
						}
						if (result.tideData.nextLow) {
							console.log(
								`⏰ Next Low: ${new Date(result.tideData.nextLow).toLocaleString("en-GB", { timeZone: "Europe/London" })} BST`
							);
						}
						...
						if (result.tideData.source === "stormglass_api") {
							console.log(
								"✅ Using StormGlass API - This is correct!"
							);
						} else if (
							result.tideData.source === "enhanced_calculation"
						) {
							console.log(
								"⚠️ Using enhanced calculation - API might not be working"
							);
						} else {
							console.log(
								"❓ Unknown source - might be using old calculation"
							);
						}
